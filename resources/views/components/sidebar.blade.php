...
<div class="flex flex-col bg-green-300 h-screen p-4 whitespace-[wrap] z-50" id="sidebar">
    ...
    <nav class="flex flex-col gap-2 h-full shadow-[inset_0px_45px_10px_-50px] w-[calc(100%+2rem)] pt-5 pl-4 pr-2 self-center">
        ...
        <a href="{{ url('records') }}" class="{{ (request()->is('records')) ? 'bg-green-500' : 'bg-white'}} flex flex-row gap-2 rounded-xl p-2 shadow-md hover:ring-2 hover:ring-green-900">
            <x-heroicon-o-user-circle class="w-5 h-auto"/> Records
        </a>
        <a href="{{ url('doctors') }}" class="{{ (request()->is('doctors')) ? 'bg-green-500' : 'bg-white'}} flex flex-row gap-2 rounded-xl p-2 shadow-md hover:ring-2 hover:ring-green-900">
            <x-heroicon-o-user-circle class="w-5 h-auto"/> Doctors
        </a>
        <a href="{{ url('patients') }}" class="{{ (request()->is('patients')) ? 'bg-green-500' : 'bg-white'}} flex flex-row gap-2 rounded-xl p-2 shadow-md hover:ring-2 hover:ring-green-900">
            <x-heroicon-o-user-circle class="w-5 h-auto"/> Patients
        </a>
